...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    body { background: #f1f5f9; margin: 0; font-family: sans-serif; overflow: hidden; }

    .sidebar { width: 220px; min-height: 100vh; background: #1a2332; position: fixed; top: 0; left: 0; display: flex; flex-direction: column; z-index: 10; }
    ...
    .main-content { margin-left: 220px; height: 100vh; display: flex; flex-direction: column; }
    .topbar { display: flex; justify-content: space-between; align-items: center; padding: 20px 28px 16px; flex-shrink: 0; }
    ...
    .chat-wrapper { display: flex; flex: 1; overflow: hidden; padding: 0 28px 28px; gap: 16px; }

    .user-list-panel { width: 300px; flex-shrink: 0; background: #fff; border-radius: 14px; border: 1px solid #e2e8f0; display: flex; flex-direction: column; overflow: hidden; }
    .user-list-header { padding: 16px; border-bottom: 1px solid #f1f5f9; }
    .user-list-title { font-size: 14px; font-weight: 600; color: #1a2332; margin: 0 0 10px; }
    .user-search { width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 12px; outline: none; color: #1a2332; }
    .user-search:focus { border-color: #22c55e; box-shadow: 0 0 0 3px #22c55e20; }
    .user-list-body { flex: 1; overflow-y: auto; }
    .user-item { display: flex; align-items: center; gap: 10px; padding: 12px 16px; cursor: pointer; border-bottom: 1px solid #f8fafc; transition: background 0.12s; }
    .user-item:hover { background: #f8fafc; }
    .user-item.active { background: #f0fdf4; border-left: 3px solid #22c55e; }
    .user-avatar { width: 38px; height: 38px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 600; color: #475569; flex-shrink: 0; }
    .user-info { flex: 1; min-width: 0; }
    .user-name { font-size: 13px; font-weight: 600; color: #1a2332; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .user-preview { font-size: 11px; color: #94a3b8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
    .user-time { font-size: 10px; color: #cbd5e1; flex-shrink: 0; }
    .user-list-empty { text-align: center; padding: 40px 20px; color: #94a3b8; font-size: 13px; }

    .chat-panel { flex: 1; background: #fff; border-radius: 14px; border: 1px solid #e2e8f0; display: flex; flex-direction: column; overflow: hidden; }
    .chat-header { padding: 14px 18px; border-bottom: 1px solid #f1f5f9; display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
    .chat-header-avatar { width: 38px; height: 38px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; font-size: 15px; font-weight: 600; color: #475569; }
    .chat-header-name { font-size: 14px; font-weight: 600; color: #1a2332; }
    .chat-header-email { font-size: 11px; color: #94a3b8; }
    .chat-header-status { margin-left: auto; font-size: 11px; color: #22c55e; background: #f0fdf4; padding: 3px 10px; border-radius: 20px; }

    .chat-messages { flex: 1; overflow-y: auto; padding: 16px 18px; display: flex; flex-direction: column; gap: 8px; background: #fafbfc; }
    ...
    .msg-sender-label { font-size: 10px; color: #22c55e; font-weight: 600; margin-bottom: 3px; }
    .msg-bubble { padding: 9px 13px; border-radius: 16px; font-size: 13px; line-height: 1.5; word-break: break-word; }
    .msg-bubble.received { background: #fff; border: 1px solid #e2e8f0; color: #1a2332; border-bottom-left-radius: 4px; }
    .msg-bubble.sent { background: #22c55e; color: #fff; border-bottom-right-radius: 4px; }
    .msg-time { font-size: 10px; color: #cbd5e1; margin-top: 3px; }
    .msg-avatar-sm { width: 28px; height: 28px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; color: #475569; flex-shrink: 0; }
    .msg-avatar-sm.admin { background: #22c55e20; color: #22c55e; }

    .chat-placeholder { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #94a3b8; gap: 10px; }
    .chat-placeholder-icon { font-size: 40px; opacity: 0.4; }
    .chat-placeholder-text { font-size: 14px; }

    .chat-input-row { padding: 12px 18px; border-top: 1px solid #f1f5f9; display: flex; gap: 10px; align-items: flex-end; flex-shrink: 0; }
    .chat-input { flex: 1; border: 1px solid #e2e8f0; border-radius: 10px; padding: 10px 14px; font-size: 13px; outline: none; resize: none; line-height: 1.4; max-height: 120px; overflow-y: auto; color: #1a2332; font-family: sans-serif; }
    .chat-input:focus { border-color: #22c55e; box-shadow: 0 0 0 3px #22c55e20; }
    .chat-input:disabled { background: #f8fafc; cursor: not-allowed; }
    .btn-send { background: #22c55e; color: #fff; border: none; border-radius: 10px; width: 42px; height: 42px; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; font-size: 16px; transition: background 0.15s; }
    .btn-send:hover { background: #16a34a; }
    .btn-send:disabled { background: #86efac; cursor: not-allowed; }

    .loading-msg { text-align: center; color: #94a3b8; font-size: 12px; padding: 20px; }
    .user-list-body::-webkit-scrollbar, .chat-messages::-webkit-scrollbar { width: 4px; }
    .user-list-body::-webkit-scrollbar-thumb, .chat-messages::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 4px; }
  </style>
</head>
<body>
...
<div class="sidebar">
  ...
  <a href="messaging.jsp" class="active">&#9993; Messages</a>
