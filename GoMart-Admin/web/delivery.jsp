...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .filter-btn { padding: 6px 16px; border-radius: 20px; border: 1px solid #e2e8f0; background: #fff; font-size: 12px; color: #64748b; cursor: pointer; transition: all 0.15s; }
    .filter-btn:hover { border-color: #22c55e; color: #22c55e; }
    .filter-btn.active { background: #22c55e; color: #fff; border-color: #22c55e; }
    .search-input { padding: 7px 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 13px; outline: none; width: 220px; }
    ...
    .order-id { font-weight: 600; color: #1a2332; }
    .customer-name { font-weight: 500; }
    .customer-address { font-size: 11px; color: #94a3b8; margin-top: 2px; }
    ...
    .delivery-select { font-size: 12px; border: 1px solid #e2e8f0; border-radius: 6px; padding: 5px 8px; color: #1a2332; outline: none; cursor: pointer; min-width: 160px; }
    .delivery-select:focus { border-color: #22c55e; }

    .btn-update { background: #22c55e; color: #fff; border: none; border-radius: 6px; padding: 5px 14px; font-size: 12px; font-weight: 500; cursor: pointer; margin-left: 6px; }
    .btn-update:hover { background: #16a34a; }
    .btn-view { background: #f8fafc; color: #475569; border: 1px solid #e2e8f0; border-radius: 6px; padding: 5px 12px; font-size: 12px; cursor: pointer; margin-left: 6px; }
    .btn-view:hover { background: #f1f5f9; }

    .alert-msg { font-size: 13px; padding: 10px 14px; border-radius: 8px; margin-bottom: 16px; display: none; }
    ...
    .modal-title { font-size: 16px; font-weight: 600; color: #1a2332; margin-bottom: 4px; }
    .modal-sub { font-size: 12px; color: #94a3b8; margin-bottom: 24px; }

    .timeline { position: relative; padding-left: 24px; }
    .timeline::before { content: ""; position: absolute; left: 7px; top: 0; bottom: 0; width: 2px; background: #f1f5f9; }
    .timeline-item { position: relative; margin-bottom: 20px; }
    .timeline-dot { position: absolute; left: -21px; width: 14px; height: 14px; border-radius: 50%; border: 2px solid #e2e8f0; background: #fff; top: 2px; }
    .timeline-dot.done { background: #22c55e; border-color: #22c55e; }
    .timeline-dot.active { background: #3b82f6; border-color: #3b82f6; }
    .timeline-label { font-size: 13px; font-weight: 500; color: #1a2332; }
    .timeline-time { font-size: 11px; color: #94a3b8; margin-top: 2px; }

    .detail-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #f8fafc; font-size: 13px; }
    .detail-row:last-child { border-bottom: none; }
    .detail-label { color: #64748b; }
    .detail-value { color: #1a2332; font-weight: 500; text-align: right; }
    .detail-section-title { font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; margin: 16px 0 10px; font-weight: 500; }
...
<div class="sidebar">
  ...
  <a href="delivery.jsp" class="active">&#9685; Delivery</a>
...
<div class="main-content">
  ...
  <div class="topbar">
    <div>
      <p class="page-title">Delivery Updates</p>
      <p class="page-sub">Track and update order delivery status</p>
    </div>
  ...
  <div class="stats-row">
    ...
    <div class="stat-card">
      <div class="stat-val" id="pendingCount" style="color:#f59e0b">--</div>
      <div class="stat-label">Pending</div>
    </div>
    <div class="stat-card">
      <div class="stat-val" id="outForDeliveryCount" style="color:#7c3aed">--</div>
      <div class="stat-label">Out for Delivery</div>
    </div>
    <div class="stat-card">
      <div class="stat-val" id="deliveredCount" style="color:#22c55e">--</div>
  ...
  <div class="content-card">
    ...
    <div class="filter-row">
      <button class="filter-btn active" onclick="filterDeliveries('all', this)">All</button>
      <button class="filter-btn" onclick="filterDeliveries('pending', this)">Pending</button>
      <button class="filter-btn" onclick="filterDeliveries('processing', this)">Processing</button>
      <button class="filter-btn" onclick="filterDeliveries('out-for-delivery', this)">Out for Delivery</button>
      <button class="filter-btn" onclick="filterDeliveries('delivered', this)">Delivered</button>
      <input type="text" class="search-input" id="searchInput"
        placeholder="Search customer..." oninput="searchDeliveries()">
    </div>
...
<div class="modal-overlay" id="deliveryModal">
  <div class="modal-box">
    <div class="modal-title" id="modalOrderId">Delivery Details</div>
    ...
    <div class="detail-row"><span class="detail-label">Contact</span><span class="detail-value" id="modalContact">—</span></div>
    ...
    <div class="modal-footer">
      <button class="btn-close-modal" onclick="closeDeliveryModal()">Close</button>
    </div>
  </div>
</div>
